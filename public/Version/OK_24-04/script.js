...
fetch('/config.json')
  ...
  .then(config => {
    ...
    console.log("✅ Config.json caricato", config);

    // Ora puoi chiamare qui eventuali funzioni che hanno bisogno di questi dati
    aggiornaBottoneConferma(); // se vuoi aggiornare subito UI iniziale
  })
  .catch(err => {
...
window.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    const [svgText, occupied] = await Promise.all([
      ...
      fetch('/occupiedSeats').then(r => r.json())
    ]);
